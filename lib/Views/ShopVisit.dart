                      ...
                      children: [
                        ...
                        ElevatedButton(
                          onPressed: isButtonPressed
                            ...
                            for (DataRow row in (filteredRows.isNotEmpty ? filteredRows : productsController.rows)) {
                              String itemDesc = row.cells[0].child?.toString() ?? '';
                              String qty = row.cells[1].child?.toString() ?? '' ;
                            //  if (qty > 0) {
...
class EditableQuantityField extends StatefulWidget {
  final dynamic? initialQuantity; // Add initialQuantity parameter
  final ValueChanged<int>? onQuantityUpdated; // Make onQuantityUpdated nullable

  const EditableQuantityField({
    Key? key,
    this.initialQuantity, // Pass initialQuantity to the constructor
  ...
  void initState() {
    ...
    _controller.addListener(() {
      if (widget.onQuantityUpdated != null) {
        widget.onQuantityUpdated!(int.tryParse(_controller.text) ?? 0);
      }
    });
  }
